...
// #if (!defined MIUNTE_TA_PASSED && !defined MIUNTE_TA_FAILED)
#ifdef MIUNTE_NO_TA
...
#define miunte_test(setup, teardown, ...)                                      \
  {                                                                            \
    ...
    size_t const test_count = sizeof(tests) / sizeof(*tests);                  \
                                                                               \
    int main_result = 0;                                                       \
    for (MiunteTest const *test = tests; test < tests + test_count; test++) {  \
      if (_setup != NULL) {                                                    \
...
#define miunte_pass()                                                          \
  { return (MiunteResult){.fn_name = __func__, .err_msg = NULL}; }             \
  ((void)0)
